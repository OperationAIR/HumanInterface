...
class Settings():
    def __init__(self, peep, freq, ratio, tidal_vol, pressure, oxygen, max_pressure, min_pressure, max_tv, min_tv, max_fio2, min_fio2):
        self.peep = int(peep)
        self.freq = int(freq)
    ...
    def get_bit_string(self):
        #B = unsigned char
        #H = unsigned short
        values = (self.peep, self.freq, self.ratio, self.tidal_vol, self.pressure, self.oxygen, self.max_pressure,self.min_pressure,self.max_tv,self.min_tv, self.max_fio2, self.min_fio2)
