...
from views.changeAlarmSettingView import AlarmType


import enum

class AlarmSettingsView(Frame):
    ...
    def fill_frame(self):
        ...
        peep_btn = FlatButton(self, self.callback, AlarmType.PEEP,
                               self.config.values['colors']['lightBlue'], fontSize=20)
        ...
        press_btn = FlatButton(self, self.callback, AlarmType.PRESSURE,
                   self.config.values['colors']['lightBlue'], fontSize=20)
        ...
        tidal_btn = FlatButton(self, self.callback, AlarmType.TIDAL,
                               self.config.values['colors']['lightBlue'], fontSize=20)
        ...
        oxy_btn = FlatButton(self, self.callback, AlarmType.OXYGEN,
                               self.config.values['colors']['lightBlue'], fontSize=20)
